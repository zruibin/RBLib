...
{
    [self mdAnimateAtPoint:point backgroundColor:backgroundColor duration:duration inflating:YES filling:filling zTopPosition:NO shapeLayer:nil completion:block];
}
...
- (void)mdDeflateAnimatedToPoint:(CGPoint)point
                 backgroundColor:(UIColor *)backgroundColor
                      ...
                      completion:(void (^)(void))block
{
    [self mdAnimateAtPoint:point backgroundColor:backgroundColor duration:duration inflating:NO filling:filling zTopPosition:NO shapeLayer:nil completion:block];
}
...
+ (void)mdInflateTransitionFromView:(UIView *)fromView toView:(UIView *)toView originalPoint:(CGPoint)originalPoint duration:(NSTimeInterval)duration completion:(void (^)(void))block {
    if (fromView.superview) {
        ...
        [containerView mdAnimateAtPoint:convertedPoint backgroundColor:toView.backgroundColor duration:duration * UIViewMaterialDesignTransitionDurationCoeff inflating:YES filling:YES zTopPosition:YES shapeLayer:nil completion:^{
            toView.alpha = 0.0;
    ...
    if (fromView.superview) {
        ...
        [UIView animateWithDuration:animationDuration
                         ...
                         completion:^(BOOL finished) {
                             ...
                             [toView mdAnimateAtPoint:convertedPoint backgroundColor:fromView.backgroundColor duration:duration * UIViewMaterialDesignTransitionDurationCoeff inflating:NO filling:YES zTopPosition:YES shapeLayer:layer completion:^{
                                 if (block) {
                                     block();
                                 }
                             }];
